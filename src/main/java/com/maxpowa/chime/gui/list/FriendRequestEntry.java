...
public class FriendRequestEntry implements GuiListExtended.IGuiListEntry {
    ...
    protected FriendRequestEntry(GuiFriendRequests parentScreen2, User user) {
        ...
        // this.resourceLocation = new ResourceLocation("users/" +
        // user.getUUID() + "/icon");
        // this.dynamicTexture =
        // (DynamicTexture)this.mc.getTextureManager().getTexture(this.resourceLocation);
    }
    ...
    public void drawEntry(int p_148279_1_, int p_148279_2_, int p_148279_3_, int p_148279_4_, int p_148279_5_, Tessellator p_148279_6_, int p_148279_7_, int p_148279_8_, boolean p_148279_9_) {
         ...
         * !this.userData.getBase64EncodedIconData().equals(this.base64Icon)) {
    ...
    /**
     * private void prepareIcon() { if (this.userData.getBase64EncodedIconData()
     * == null) {
     * this.mc.getTextureManager().deleteTexture(this.resourceLocation);
     * this.dynamicTexture = null; } else { ByteBuf bytebuf =
     * Unpooled.copiedBuffer(this.userData.getBase64EncodedIconData(),
     * Charsets.UTF_8); ByteBuf bytebuf1 = Base64.decode(bytebuf); BufferedImage
     * bufferedimage; label74: { try { bufferedimage = ImageIO.read(new
     * ByteBufInputStream(bytebuf1));
     * Validate.validState(bufferedimage.getWidth() == 64,
     * "Must be 64 pixels wide", new Object[0]);
     * Validate.validState(bufferedimage.getHeight() == 64,
     * "Must be 64 pixels high", new Object[0]); break label74; } catch
     * (Exception exception) { logger.error("Invalid icon for user " +
     * this.userData.getUsername() + " (" + this.userData.getUUID() + ")",
     * exception); this.userData.func_147407_a((String)null); } finally {
     * bytebuf.release(); bytebuf1.release(); }
     * 
     * return; }
     * 
     * if (this.dynamicTexture == null) { this.dynamicTexture = new
     * DynamicTexture(bufferedimage.getWidth(), bufferedimage.getHeight());
     * this.mc.getTextureManager().loadTexture(this.resourceLocation,
     * this.dynamicTexture); }
     * 
     * bufferedimage.getRGB(0, 0, bufferedimage.getWidth(),
     * bufferedimage.getHeight(), this.dynamicTexture.getTextureData(), 0,
     * bufferedimage.getWidth()); this.dynamicTexture.updateDynamicTexture(); }
     * }
     **/
