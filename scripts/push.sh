...
JSON=`printf '{"version": { "name": "%s.%s", "minecraft": "%s", "changelog": "See commit [url=http://github.com/maxpowa/chime/commits/%s]%s[/url] for full change details.", "tag": "%s" }, "filename": "%s" }' "$MAJOR_MINOR" "$BUILD_NUMBER" "$MC_VERSION" "$COMMIT" "$COMMIT" "$TAG" "$FILE_NAME"`
