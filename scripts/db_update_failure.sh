...
TEMP_JSON=`printf '{"SUCCESS":false,"FORGEVERSION":"%s","COMMIT":"%s","JAVA_VERSION":"%s","BRANCH":"%s","JOB_ID":"%s","PULL_REQUEST":"%s","START_TIME":"%s","END_TIME":"%s","PUSH":"%s"}\n' "$FORGEVERSION" "$COMMIT" "$JAVA_VERSION" "$BRANCH" "$JOB_ID" "$PULL_REQUEST" "$START_TIME" "$CURRENT_TIME" "$PUSH"` 
